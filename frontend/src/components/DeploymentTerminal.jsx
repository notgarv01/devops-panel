...
export default function DeploymentTerminal({ sessionId, title = 'Deployment Terminal', autoConnect = true }) {
  ...
  return (
    <div className={`bg-[#0F0F0F] border border-zinc-800 rounded-2xl flex flex-col overflow-hidden transition-all duration-300 ${
      isExpanded ? 'h-[75vh] sm:h-[600px]' : 'h-80 sm:h-96'
    }`}>
      {/* Header */}
      <div className="p-3 sm:p-4 border-b border-zinc-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-zinc-900/30">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          ...
          {sessionId && (
            <span className="hidden md:inline text-xs text-zinc-600 font-mono flex-shrink-0">
      ...
      <div
        ...
        className="flex-1 overflow-y-auto p-3 sm:p-4 font-mono text-sm bg-[#050505] select-text scroll-smooth"
      >
        {logs.length > 0 ? (
      ...
      {/* Status Bar */}
      <div className="px-3 sm:px-4 py-2 border-t border-zinc-800 bg-zinc-900/50 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
        <div className="flex items-center gap-4 min-w-0">
          <span className="text-zinc-500">
            Lines: <span className="text-zinc-400">{logs.length}</span>
          </span>
          <span className="text-zinc-500">
            Session: <span className="text-zinc-400 font-mono break-all">{sessionId || 'N/A'}</span>
          </span>
        </div>
