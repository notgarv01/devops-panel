...
const generateVercelConfig = (projectType, config = {}) => {
  ...
  if (projectType === PROJECT_TYPES.NODE_API || projectType === 'NODE_API' || projectType === 'MERN') {
    ...
    // ✅ FIX: installCommand installs backend deps for serverless function
    return {
      version: 2,
...
const injectServerlessBridge = async (workDir, audit) => {
  ...
  // MUST use ../ to go up from api folder to root, then into backend
  // Use dynamic import() for ES Module compatibility
  // Connect to MongoDB before Express handles the request
...
`;
  ...
  // NOTE: Do NOT create api/package.json - Vercel treats it as isolated sandbox
  // Vercel will use the root package.json instead (set by installCommand)

  // Copy backend dependencies for root package.json
  ...
  if (fs.existsSync(backendPkgPath)) {
    try {
      ...
      rootDependencies = {
        express: backendPkg.dependencies?.express || '*',
        mongoose: backendPkg.dependencies?.mongoose || '*',
        cors: backendPkg.dependencies?.cors || '*',
        'cookie-parser': backendPkg.dependencies?.['cookie-parser'] || '*',
        dotenv: backendPkg.dependencies?.dotenv || '*'
      };
    } catch (e) {
      console.log('[Transform] Could not read backend package.json:', e.message);
    }
  }

  // Also create a root package.json so Vercel installs all needed dependencies
  const rootPackageJson = {
    name: 'gupta-sales-devops',
    version: '1.0.0',
    dependencies: rootDependencies
  };
  await fsp.writeFile(path.join(workDir, 'package.json'), JSON.stringify(rootPackageJson, null, 2));

  console.log('[Transform] === STEP 6: SERVERLESS ENTRY CREATED ===');
  console.log('  Bridge: api/index.js → ../backend/src/app (ESM)');
  console.log('  Dependencies: express, mongoose, cors, cookie-parser');
  console.log('  Root package.json created for Vercel');
...
module.exports = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: "APP_RUNTIME_ERROR", message: error.message });
  }
};`;
...
const generateTailoredConfig = async (workDir, audit) => {
  ...
  // Build command must install BOTH frontend AND api dependencies for serverless to work
  const buildCommand = frontendDir
    ? `cd ${frontendDir} && npm install && ${buildScript} && cd ../api && npm install`
  ...
  // ✅ FIX: installCommand installs backend deps first, then buildCommand builds frontend
  const vercelConfig = {
    version: 2,
...
const transformForDeployment = async (workDir, projectType, options = {}) => {
  ...
  if (projectType === 'MERN' || projectType === 'FRONTEND') {
    ...
    if (hasFrontend && hasBackend && discovery.architectRecommendations?.length > 0) {
